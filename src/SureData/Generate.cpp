...
void SureData::GenerateParticle(SureObject *i_ps,my_double3 i_X,double i_size,my_double3 i_vec)
{
    if(i_ps->ParticlesCounter>=i_ps->ParticlesLimit)return;
    i_ps->Particles[i_ps->ParticlesCounter].X = i_X;
    i_ps->Particles[i_ps->ParticlesCounter].OX = i_ps->Particles[i_ps->ParticlesCounter].X - i_vec;
    i_ps->Particles[i_ps->ParticlesCounter].Size = i_size;
    ++i_ps->ParticlesCounter;
}

void SureData::SetParticlesNumber(SureObject *i_ps,int i_number)
{
    if(i_ps->type == SURE_OBJ_PS){
        if(i_ps->ParticlesInitiated)delete [] i_ps->Particles;
        i_ps->Particles = new SureParticle[i_number];
        i_ps->ParticlesInitiated = true;
        i_ps->ParticlesCounter = 0;
        i_ps->ParticlesLimit = i_number;
    };
}

void SureData::GenerateParticleCloud(SureObject *i_ps,int i_number)
{
    if(i_ps->type == SURE_OBJ_PS){
        i_ps->Particles = new SureParticle[i_number];
        i_ps->ParticlesInitiated = true;
        for(int i = 0;i<i_number;++i){
            i_ps->Particles[i].X.x = 2.0 *((float)rand()/(float)RAND_MAX - 0.5);
            i_ps->Particles[i].X.y = 2.0 *((float)rand()/(float)RAND_MAX - 0.5);
            i_ps->Particles[i].X.z = 2.0 *((float)rand()/(float)RAND_MAX - 0.5);
            i_ps->Particles[i].OX = i_ps->Particles[i].X;
            i_ps->Particles[i].Size = 15.0 + 10.0 * (float)rand()/(float)RAND_MAX;
        };
        i_ps->ParticlesCounter = i_number;
        i_ps->ParticlesLimit = i_number;
    };
}
